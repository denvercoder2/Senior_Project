...
public class SkyMap_Formulae_J {
	...
	public static ArrayList getSpace() throws NullPointerException, Exception {
		...
		// This creates the SpaceObj for luna
		SpaceObj tempObjForLuna = new SpaceObj();
        ArrayList<String> MoonInfo = Moon.getPhase(Calendar.MONTH, Calendar.DAY_OF_MONTH, Calendar.YEAR);
		tempObjForLuna.setType(MoonInfo.get(0));
		tempObjForLuna.setProperName("MOON");
		tempObjForLuna.setMagnitude(MoonInfo.get(1));
		spaceObjList.add(tempObjForLuna);
		...
		ArrayList<JythonThreader> threadList = new ArrayList<JythonThreader>();
		...
		for (int k = 0; k < spaceObjList.size(); k++) {
			...
			if (spaceObjList.get(k).getProperName() == "mercury")
				...
				argument11 = "sol";
			else if (spaceObjList.get(k).getProperName() == "MOON")
				argument11 = "luna";
			...
			spaceObjList.get(k).setAzimuth(Double.parseDouble(outputStr.substring(outputStr.lastIndexOf(',')+2,outputStr.lastIndexOf(')'))));*/
				...
				JythonThreader object = new JythonThreader(arguments, spaceObjList.get(k), k);
				
				object.start();
				object.join();
				
				spaceObjList.get(k).setAzimuth(object.azimuth);
				spaceObjList.get(k).setAltitude(object.altitude);
			//spaceObjList.get(k).setAltitude(Double.parseDouble(temp.substring(0, temp.charAt(':')-1)));
			//spaceObjList.get(k).setAltitude(Double.parseDouble(temp.substring(temp.charAt(':')+1, temp.length())));
		}
	...
	*/	
		for (int i = 0; i < spaceObjList.size(); i++) {
			if (spaceObjList.get(i).getProperName() != null && spaceObjList.get(i).getProperName() != "" && spaceObjList.get(i).getProperName() == "MOON") {
				System.out.println(spaceObjList.get(i).getProperName());
	...
	public static ArrayList getSpace(String YEAR, String MONTH, String DAY, String HOUR, String MIN, String SEC) throws NullPointerException, Exception {
		Calendar c = Calendar.getInstance(); 
		...
		// This creates the SpaceObj for luna
		SpaceObj tempObjForLuna = new SpaceObj();
        ArrayList<String> MoonInfo = Moon.getPhase(Integer.parseInt(MONTH), Integer.parseInt(DAY), Integer.parseInt(YEAR));
		tempObjForLuna.setType(MoonInfo.get(0));
		tempObjForLuna.setProperName("MOON");
		tempObjForLuna.setMagnitude(MoonInfo.get(1));
		spaceObjList.add(tempObjForLuna);
	...
	public static ArrayList getSpace(String YEAR, String MONTH, String DAY, String HOUR, String MIN, String SEC, String LAT, String LONG) throws NullPointerException, Exception {
		Calendar c = Calendar.getInstance(); 
	...
	public static ArrayList getSpace(String LAT, String LONG) throws NullPointerException, Exception {
		Calendar c = Calendar.getInstance(); 
